...
if [ ! -e $dir -o ! -e $full_path ]; then
    echo "[CHOI : Build Only Rom For Dev Process(Android)] Pre Check :: $target_version Not Exist."
    exit 1;
fi
...
echo ">>>> [CHOI : Build Only Rom For Release Process(Android)] build start! : build_tag=$build_tag"

# UnityHubによって、Unity側のビルドスクリプト実行
$launch -projectPath $parent_path -executeMethod Choi.MyProj.API.Editor.Build.BuildAPI.BuildOnlyRomReleaseAndroid -batchmode -quit -logFile $log_path -buildTarget android /build_tag $build_tag
...
# 正常終了コードを返して終了
echo ">>>> [CHOI : Build Only Rom For Release Process(Android)] build Finished!"
exit 0
