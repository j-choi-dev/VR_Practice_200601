...
if [ ! -e $dir -o ! -e $full_path ]; then
    echo "[CHOI : Build Only Release Process(iOS)] Pre Check :: $target_version Not Exist."
    exit 1;
fi
...
echo ">>>> [CHOI : Build Rom Only Release Process(iOS)] build start! : build_tag=$build_tag"

# UnityHubによって、Unity側のビルドスクリプト実行
$launch -projectPath $parent_path -executeMethod Choi.MyProj.API.Editor.Build.BuildAPI.BuildOnlyRomReleaseIos -batchmode -quit -logFile $log_path -buildTarget iOS /build_tag $build_tag
...
# 正常終了コードを返して終了
echo ">>>> [CHOI] Build Only Rom For Release Process(iOS)] IPA Export Finished!"
echo ">>>> [CHOI : Build Only Rom For Release Process(iOS)] build Finished!"
exit 0;