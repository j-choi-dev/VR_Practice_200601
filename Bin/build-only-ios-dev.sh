...
if [ ! -e $dir -o ! -e $full_path ]; then
    echo "[CHOI : Build Only Rom For Dev Process(iOS)] Pre Check :: $target_version Not Exist."
    exit 1;
fi
...
echo ">>>> [CHOI : Build Only Rom For Dev Process(iOS)] build start! : build_tag=$build_tag"

# UnityHubによって、Unity側のビルドスクリプト実行
$launch -projectPath $parent_path -executeMethod Choi.MyProj.API.Editor.Build.BuildAPI.BuildOnlyRomDevIos -batchmode -quit -logFile $log_path -buildTarget iOS /build_tag $build_tag
...
exportResult=$(source ./ipa-export.sh $build_tag)
...
# 正常終了コードを返して終了
echo ">>>> [CHOI : Build Only Rom For Dev Process(iOS)] IPA Export Finished!"
echo ">>>> [CHOI : Build Only Rom For Dev Process(iOS)] build Finished!"
exit 0;