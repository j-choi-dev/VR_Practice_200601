...
if [ ! -e $dir -o ! -e $full_path ]; then
    echo "[CHOI : Build All for Dev Process(iOS)] Pre Check :: $target_version Not Exist."
    exit 1;
fi
...
echo ">>>> [CHOI : Build All for Dev Process(iOS)] build start! : build_tag=$build_tag"
...
# 正常終了コードを返して終了
echo ">>>> [CHOI : Build All for Dev Process(iOS)] IPA Export Finished!"
exit 0;